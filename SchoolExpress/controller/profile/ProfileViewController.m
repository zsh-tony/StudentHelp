...
- (void)viewDidLoad
{
    ...
    //if ([AccountTool sharedAccountTool].currentAccount.uid == nil) {
        //loginViewController *login = [[loginViewController alloc]init];
        //[self presentViewController:login animated:YES completion:nil];
    //}else{
    
    //[[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(pushMyOrderVC:) name:@"pushMyOrderVC" object:nil];
    self.title = @"我的";
    [self initTableView];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section == 0) {
        ...
        //imageView.image = [UIImage imageNamed:@"avatar_default_big.png"];
        //    personIconCell.nameSexBtn.titleLabel.textColor = [UIColor blackColor];
        //    [personIconCell.nameSexBtn setTitle:@"阿邦♂" forState:UIControlStateNormal];
...
//            messageCell = [tableView                               dequeueReusableCellWithIdentifier:CellIdentifier2];
//            if (messageCell == nil) {
//                messageCell = [[MessageCell alloc]initWithStyle:UITableViewCellStyleDefault                   reuseIdentifier:CellIdentifier2] ;
//            }
//            messageCell.selectionStyle = UITableViewCellSelectionStyleNone;
//            messageCell.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
//            messageCell.countLabel.text = [NSString stringWithFormat:@"%d",3];
//            messageCell.summaryLabel.text = @"我哦阿胶为服务费";
//            messageCell.creatAtLabel.text = @"15:20";
//            messageCell.iconImageView.image = [UIImage imageNamed:@"avatar_default_big.png"];
//            return messageCell;
//        }
    }else if(indexPath.section == 1){
        if (indexPath.row == 0) {
