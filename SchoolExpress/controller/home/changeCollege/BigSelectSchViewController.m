...
@interface BigSelectSchViewController ()
{
...
@implementation BigSelectSchViewController
...
- (void)addScrollView
{
    ...
    _scrollview.defaultsch = 1;
    _scrollview.btnArray = btnArrayData;
    
    //NSLog(@"%d",_scrollview.defaultsch);
    __weak BigSelectSchViewController *weakSelf = self;
    _scrollview.selectSchBlock = ^(int selectSch,NSString *display){
        //1、传递defaultsch参数
        if (weakSelf.selectmidSchBlock) {
            weakSelf.selectmidSchBlock(selectSch,display);
            
        }
    };
    [_scrollview setBackgroundColor:kGlobalBg];
    _scrollview.delegate = self;
    _scrollview.delaysContentTouches = NO;
    _scrollview.contentSize = [SchBtnScrollView btnSizeWidthCount:(int)btnArrayData.count];
...
- (void)addOtherView
{
    ...
    _middleView.backgroundColor = kGlobalBg;
...
-(void)selectCity
{
    ...
    __block UIButton *weakCityBtn = cityBtn;
    changeCityViewControl.changeCityBlock = ^(int changeCity,NSString *display){
        
        [weakCityBtn setTitle:[NSString stringWithFormat:@"当前城市：%@",display] forState:UIControlStateNormal];
...
- (void)viewDidLoad
{
    ...
    btnArrayData = @[@"全部",@"本部",@"雁塔",@"小寨",@"渭水",@"sas",@"dsafsa",@"dsl",@"we",@"owie",@"dsa",@"wew"];
